...
function main() {
  ...
  }
  {
    ...
    mtlLoader.load("objects/rumah_hantu.mtl", (mtl) => {
      mtl.preload();
      const objLoader = new OBJLoader();
      objLoader.setMaterials(mtl);
      objLoader.load("objects/rumah_hantu.obj", (root) => {
        root.rotation.x = -Math.PI * 0.5;
        root.castShadow = true;
        root.receiveShadow = true;
        root.traverse(function (child) {
...

  {
...

    {
      ...
      folder.open();
      var minMaxGUIHelper = new MinMaxGUIHelper(
        light.shadow.camera,
        "near",
        "far",
        0.1
      ...
      camFolder
        ...
        .onChange(updateCamera);
      camFolder
        .add(minMaxGUIHelper, "max", 0.1, 300, 0.1)
        .name("far")
        .onChange(updateCamera);
    }
